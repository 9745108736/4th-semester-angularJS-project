...
$first = "select m_id from member_add where phone = '$phone'";
...
$first = "select phone from member_add where m_id= '$affiliation_number'";
$result = mysqli_query($connection,$first);
$count = mysqli_num_rows($result);    
if($count == 0)
{
...
else
{
	...
	echo "<script>alert('Directing to password change page')</script>";
  	echo "<script>document.location.href='m_forget_password1.php'</script>";
	exit;
}
